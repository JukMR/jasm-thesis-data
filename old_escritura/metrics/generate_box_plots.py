...
class BoxplotGenerator:
    @staticmethod
    def generate_separate_boxplots(data: dict, output_filename: Path) -> None:
...
def generate_boxplots(paths: List[Path], output_filename: Path) -> None:
    execution_times: Dict[Path, np.array] = {}
    ...
    if len(paths) == 1:
        ...
        BoxplotGenerator().generate_separate_boxplots(execution_times, output_filename)
